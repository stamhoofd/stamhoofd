...
export function useGlobalRoutes() {
    ...
    onCheckRoutes(async () => {
        ...
        if (initialParts.length >= 1 && initialParts[0] === 'verify-email') {
            const token = initialQueryString.get('token');
            const code = initialQueryString.get('code');

            if (token && code) {
                const toast = new Toast($t(`%IZ`), 'spinner').setHide(null).show();

                try {
                    const session = initialParts[1] ? await SessionContext.createFrom({ organizationId: initialParts[1] }) : new SessionContext(null);
