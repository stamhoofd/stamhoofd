<template>
    <figure class="style-icon-with-progress" :class="{complete: isComplete}">
        <figure>
            <div v-if="group" class="primary-icon-container">
                <ImageComponent v-if="group.settings.squarePhoto ?? group.settings.coverPhoto" :image="group.settings.squarePhoto ?? group.settings.coverPhoto" />
                <span v-else>{{ group.settings.getShortCode(2) }}</span>
            </div>
            <div v-else class="primary-icon-container">
