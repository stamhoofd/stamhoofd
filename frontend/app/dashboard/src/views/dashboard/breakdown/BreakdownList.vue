<template>
    <STGrid>
        <STGridItem v-for="group of groups" :key="group.id" :selectable="canOpen(group)" class="price-grid right-stack breakdown-row" @click="canOpen(group) && $emit('select', group)">
            <template #left>
