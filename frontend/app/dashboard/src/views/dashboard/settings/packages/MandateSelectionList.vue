...
import { OrganizationPaymentMandate, STBillingStatus } from '@stamhoofd/structures';
...
const selectedMandate = defineModel<string | null>({ default: null });
...
function isDeleting(mandate: OrganizationPaymentMandate): boolean {
    return deletingMandates.includes(mandate.id);
}
...
async function deleteMandate(mandate: OrganizationPaymentMandate) {
    ...
    deletingMandates.push(mandate.id);
    ...
    // Remove
    deletingMandates = deletingMandates.filter(id => id !== mandate.id);
}
