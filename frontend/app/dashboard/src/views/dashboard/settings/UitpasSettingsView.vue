                ...
                >
                    ...
                    <template #right>
                        <span v-if="uitpasOrganizerId" class="button text">
                            {{ uitpasClientCredentialsStatus === UitpasClientCredentialsStatus.NOT_CONFIGURED ? $t('Configureer') : $t('Wijzig') }}
                            <span class="icon arrow-right-small" />
                        </span>
                    </template>
                </STListItem>
...
const { patched, hasChanges, addPatch, patch } = usePatch(organizationManager.value.organization);
    ...
    await show(
        new ComponentWithProperties(SearchUitpasOrganizerView, {
            ...
            selectOrganizer: async (organizer: UitpasOrganizerResponse, { pop }: NavigationActions) => {
                const oldId = uitpasOrganizerId.value;
    ...
    present({
        components: [
            new ComponentWithProperties(SetUitpasClientCredentialsView, {
                saveHandler: (newStatus: UitpasClientCredentialsStatus) => {
                    // uitpasClientCredentialsStatus.value = newStatus;
                },
            }),
        ],
