<template>
    <SaveView :title="title" :disabled="!hasChanges && !isNew" class="edit-seating-plan-view" :loading="saving" @save="save">
        ...
        <STInputBox error-fields="name" :error-box="errors.errorBox" :title="$t(`%Uf`)">
            <input ref="firstInput" v-model="name" class="input" type="text" :placeholder="$t(`%Ug`)" autocomplete="off" enterkeyhint="next">
        </STInputBox>
