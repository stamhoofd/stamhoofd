...
import { CartItem, CartItemOption, CheckoutMethodType, Gender, getGenderName, OrderStatusHelper, PaymentMethodHelper, ProductType, ReservedSeat, TicketPublic } from '@stamhoofd/structures';
...
function getAnswerGroups<R>(webshop: Webshop, orders: PrivateOrder[], getOrder: (row: R) => PrivateOrder): OrdersExcelColumnGroup<R>[] {
    ...
    // Records configured on the webshop
    for (const record of webshop.meta.recordCategories.flatMap(category => category.getAllRecords())) {
