    ...
    <SaveView :loading="saving" :title="title" :disabled="!hasChanges" @save="save" v-on="canDeleteOrRename && !isNew && !isRoot ? {delete: deleteMe} : {}">
        ...
        <template v-if="!isRoot">
            <STInputBox v-if="canDeleteOrRename" error-fields="name" :error-box="errors.errorBox" :title="$t(`%1Os`)">
                <input ref="firstInput" v-model="name" v-autofocus="isNew" class="input" type="text" autocomplete="off" :placeholder="$t(`%LT`)">
            </STInputBox>
...
async function deleteMe() {
    if (!await CenteredMessage.confirm({
        title: groups.value.length
            ? $t('%ZmQ', { categoryName: name.value })
            : $t('%ZnW', { categoryName: name.value }),
        confirmText: $t('%CJ'),
