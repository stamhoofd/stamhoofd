...
export class EmailStyler {
    static async format(html: string, subject: string, organization: Organization): Promise<{ text: string; html: string }> {
        ...
        const scss = ""; //new URL('./email.scss', import.meta.url).toString();
        await Promise.resolve()
        //((await import(/* webpackChunkName: "email-css" */ "!!asset/source!sass-loader!./email.scss")).default as string).replaceAll("#0053ff", primaryColor)
