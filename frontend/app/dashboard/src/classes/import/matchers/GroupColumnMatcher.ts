...
export class GroupColumnMatcher implements ColumnMatcher {
    ...
    doesMatch(columnName: string, examples: string[]): boolean {
        ...
        const possibleMatch = ["groep", "tak", "indeling", "categorie", "ploeg", "functie"]
    ...
    apply(cell: XLSX.CellObject, member: ImportingMember) {
        ...
        if (!minErrorGroup) {
            /*throw new SimpleError({
                code: "invalid_type",
                message: "'"+ value +"' is geen geldige leeftijdsgroep (deze bestaat niet in Stamhoofd). Kijk na of er geen fout in zit.",
            })*/
            console.warn("'"+ value +"' is geen geldige leeftijdsgroep (deze bestaat niet in Stamhoofd). Kijk na of er geen fout in zit.")
        }
    }
}