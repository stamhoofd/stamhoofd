...
import { AccessRight, ContinuousMembershipStatus, Group, GroupCategoryTree, GroupPrice, GroupType, MemberResponsibility, MembershipStatus, MemberWithRegistrationsBlob, Organization, PermissionLevel, PlatformRegistration, RecordAnswer, RegisterItemOption, StamhoofdFilter } from '@stamhoofd/structures';
...
const allColumns: Column<ObjectType, any>[] = [
    ...
    new Column<ObjectType, string>({
        ...
        name: '#',
        getValue: registration => getMember(registration).details.memberNumber ?? '',
    ...
    new Column<ObjectType, string>({
        ...
        getValue: registration => getMember(registration).name,
        minimumWidth: 100,
    ...
    new Column<ObjectType, Date | null>({
        ...
        getValue: registration => getMember(registration).details.birthDay,
    ...
    new Column<ObjectType, number | null>({
        ...
        getValue: registration => getMember(registration).details.age,
    ...
    props.dateRange !== null
        ? new Column<ObjectType, ContinuousMembershipStatus>({
            ...
            getValue: registration => registration.member.getContinuousMembershipStatus(props.dateRange!),
            format: (status) => {
                switch (status) {
    ...
    new Column<ObjectType, string[]>({
        ...
        allowSorting: false,
        getValue: registration => registration.member.getResponsibilities({ organization: props.organization ?? undefined }).map(l => l.getName(registration.member, false)),
        format: (list) => {
            if (list.length === 0) {
    ...
    new Column<ObjectType, string[]>({
        ...
        allowSorting: false,
        getValue: registrations => registrations.member.patchedMember.users.filter(u => u.hasAccount).map(u => u.email),
        format: (accounts) => {
    ...
    if (props.group.settings.prices.length > 1) {
        allColumns.push(
            new Column<ObjectType, GroupPrice[]>({
                ...
                getValue: registration => registration.member.filterRegistrations({ groups: [props.group!] }).map(r => r.groupPrice),
    ...
    for (const optionMenu of props.group.settings.optionMenus) {
        allColumns.push(
            new Column<ObjectType, RegisterItemOption[]>({
                ...
                getValue: registration => registration.member.filterRegistrations({ groups: [props.group!] }).flatMap((r) => {
    ...
    allColumns.push(
        new Column<ObjectType, string[]>({
            ...
            getValue: (registration) => {
                ...
                if (!registration.member.patchedMember.details.phone && registration.member.isPropertyRequired('phone', scope)) {
                    base.push($t(`de723a38-6e76-418a-a6f6-52c6ed45c5c8`));
                }

                if (!registration.member.patchedMember.details.email && registration.member.isPropertyRequired('emailAddress', scope)) {
                    base.push($t(`64163c88-2610-4542-9fd4-db523670049c`));
                }

                if (!registration.member.patchedMember.details.address && registration.member.isPropertyRequired('address', scope)) {
                    base.push($t(`ca287035-d735-4eaa-bbb3-ae0db435b4ea`));
                }

                if (!registration.member.patchedMember.details.birthDay && registration.member.isPropertyRequired('birthDay', scope)) {
                    base.push($t(`88a24a2b-d84a-4c7e-978d-6180e260a06f`));
                }

                if (!registration.member.patchedMember.details.nationalRegisterNumber && registration.member.isPropertyRequired('nationalRegisterNumber', scope)) {
                    base.push($t(`e7a21ff5-4f90-4518-8279-ea4fb747fb66`));
                }
                else {
                    if (registration.member.isPropertyRequired('parents', scope) && registration.member.isPropertyRequired('nationalRegisterNumber', scope) && !registration.member.patchedMember.details.parents.find(p => p.nationalRegisterNumber)) {
                        base.push($t(`af59b3e6-e47c-4c9f-a571-2e1662f17114`));
                    }
                }

                if (registration.member.isPropertyRequired('parents', scope)) {
                    if (registration.member.patchedMember.details.parents.length === 0) {
                        base.push($t(`8a5dfcff-bbe3-4b8f-8b6d-85df4d35dc94`));
                    }
                }

                if (registration.member.patchedMember.details.emergencyContacts.length === 0 && registration.member.isPropertyRequired('emergencyContacts', scope)) {
                    base.push($t(`d42f4d7d-a453-403b-9b3f-459020fc8849`));
                }

                const { categories: enabledCategories } = registration.member.getEnabledRecordCategories(scope);
...
if (props.group && props.group.type === GroupType.EventRegistration && props.group.settings.allowRegistrationsByOrganization) {
    allColumns.push(
        new Column<ObjectType, string | null>({
            ...
            getValue: (registration) => {
                const registrations = registration.member.filterRegistrations({ groups, periodId: filterPeriodId });
                if (registrations.find(r => r.payingOrganizationId)) {
                    const organization = registration.member.organizations.find(o => o.id === registrations[0].payingOrganizationId);
...
allColumns.push(
    new Column<ObjectType, Date | null>({
        ...
        allowSorting: false,
        getValue: (v) => {
            const registrations = v.member.filterRegistrations({ groups, periodId: filterPeriodId });
...
if (!waitingList.value && financialRead.value) {
    allColumns.push(
        new Column<ObjectType, number>({
            ...
            allowSorting: false,
            getValue: v => v.member.filterRegistrations({ groups: groups }).flatMap(r => r.balances).reduce((sum, r) => sum + (r.amountOpen + r.amountPaid + r.amountPending), 0),
    ...
    allColumns.push(
        new Column<ObjectType, number>({
            ...
            allowSorting: false,
            getValue: v => v.member.filterRegistrations({ groups: groups }).flatMap(r => r.balances).reduce((sum, r) => sum + (r.amountOpen), 0),
            format: (outstandingBalance) => {
                if (outstandingBalance < 0) {
                    return Formatter.price(outstandingBalance);
                }
    ...
    allColumns.push(
        new Column<ObjectType, Group[]>({
            ...
            getValue: (v) => {
                ...
                const memberGroups = v.member.filterGroups({ groups: groups, periodId: filterPeriodId });
...
if (!props.group && !props.category) {
    allColumns.push(
        new Column<ObjectType, Group[]>({
            ...
            getValue: (v) => {
                let memberGroups = v.member.filterGroups({ periodId: filterPeriodId, types: [GroupType.Membership, GroupType.WaitingList] });
                if (app === 'admin') {
