<template>
    <STListItem v-long-press="(e: MouseEvent) => showContextMenu(e)" :selectable="true" class="right-stack" @click="editCategory()" @contextmenu.prevent="showContextMenu">
        ...
        <template #right>
            <span v-if="category.containsSensitiveData" v-tooltip="$t('%jJ')" class="icon tiny privacy gray" />
...
async function deleteCategory() {
    if (!await CenteredMessage.confirm({
        title: props.category.name.toString() ? $t('%Zm2', { name: props.category.name.toString() }) : $t('Deze vragenlijst verwijderen?'),
        confirmText: $t(`%CJ`),
        description: $t(`%11j`) + ' ' + $t('%Zmm'),
