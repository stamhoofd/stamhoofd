<template>
    <SaveView :title="title" :disabled="!hasChanges" :save-text="$t('Opslaan')" @save="save">
        ...
        <STList v-else>
            <ResourcePermissionRow v-for="resource in filteredResources" :key="resource.id" :role="patched" :inherited-roles="inheritedRoles" :resource="resource" :configurable-access-rights="configurableAccessRights" type="resource" :ignore-general-access="true" @patch:role="addPatch" />
        </STList>
    </SaveView>
</template>
