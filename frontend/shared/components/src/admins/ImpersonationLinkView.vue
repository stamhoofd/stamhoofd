<template>
    <SaveView :loading="loading" :title="$t(`Aanmelden als gebruiker`)" :disabled="!link" :save-text="$t(`Link kopiëren`)" save-icon="copy" data-testid="impersonation-link-view" @save="copy">
        <h1>{{ $t('Aanmelden als {name}', {name: userName}) }}</h1>
        <p>{{ $t('Met deze link bekijk je het platform zoals deze gebruiker het ziet. Je blijft zelf verantwoordelijk: alles wat je wijzigt, wordt geregistreerd op jouw naam.') }}</p>

        <STErrorsDefault :error-box="errors.errorBox" />

        <p class="warning-box">
            {{ $t('Open deze link in een privévenster (incognito). In een gewoon venster meld je jezelf af.') }}
        </p>

        <template v-if="link">
            <STInputBox :title="$t(`Link`)" error-fields="link" :error-box="errors.errorBox">
