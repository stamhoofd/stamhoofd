<template>
    <div class="st-view mfa-settings-view" data-testid="mfa-settings-view">
            ...
            <template v-if="status">
                ...
                <STList>
                    <STListItem v-for="totp of status.totp" :key="totp.id" data-testid="totp-item">
                        ...
                        <p class="style-description-small">
                            {{ $t('Toegevoegd op {date}', { date: Formatter.date(totp.createdAt) }) }}
                        </p>
                        <template #right>
                            <LoadingButton>
                ...
                <STList>
                    <STListItem v-for="passkey of status.passkeys" :key="passkey.id" data-testid="passkey-item">
                        ...
                        <p class="style-description-small">
                            {{ $t('Toegevoegd op {date}', { date: Formatter.date(passkey.createdAt) }) }}
                        </p>
                        <template #right>
                            <LoadingButton>
...
async function addPasskey() {
    errors.errorBox = null;
    try {
        const result = await runFresh(() => MFAManager.registerPasskey($context.value, $t('Passkey')));
        ...
        if (result.recoveryCodes && result.recoveryCodes.codes.length > 0) {
            ...
            Toast.success($t('Passkey toegevoegd')).show();
        }
    }
    catch (e) {
        errors.errorBox = new ErrorBox(e);
