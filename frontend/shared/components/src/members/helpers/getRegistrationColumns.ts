import { AppType, Column } from '@stamhoofd/components';
import { ContextPermissions } from '@stamhoofd/networking';
import { ContinuousMembershipStatus, Group, GroupCategoryTree, GroupPrice, GroupType, MembershipStatus, Organization, PermissionLevel, PlatformRegistration, RecordAnswer, RegisterItemOption } from '@stamhoofd/structures';
    ...
    if (!waitingList && financialRead) {
        ...
        allColumns.push(
            new Column<ObjectType, number>({
                ...
                allowSorting: false,
                getValue: (registration) => {
                    return registration.balances.reduce((sum, r) => sum + (r.amountOpen), 0);
                },
                format: (outstandingBalance) => {
                    if (outstandingBalance < 0) {
                        return Formatter.price(outstandingBalance);
