...
test('CodeInput normalizes characters, formats with dashes, updates the model, and emits complete', async () => {
    ...
    // The remaining characters of the mask are rendered as one span per character
    const suffix = () => Array.from(document.querySelectorAll('.code-input .suffix')).map(el => el.textContent).join('');
