...
const currentPeriod = computed(() => isPlatform ? patchedPlatform.value.period : (patched.value.find(p => p.id === organization.value.period.period.id) ?? organization.value.period.period));
