<template>
    <LoadingViewTransition :loading="isInitialLoading">
        <SaveView :title="$t('UiTPAS-integratie')" class="edit-sponsor-view" :loading="isSaveLoading" @save="save">
            ...
            <STInputBox error-fields="clientSecret" :error-box="errors.errorBox" :title="$t('Client id')">
...
import { UitpasClientIdAndSecret, UitpasClientCredentialsStatus } from '@stamhoofd/structures';
...
const props = defineProps<{
    saveHandler: (status: UitpasClientCredentialsStatus) => Promise<void>;
}>();
...
async function save() {
    ...
    try {
        ...
        if (human) {
            ...
            await props.saveHandler(newStatus);
