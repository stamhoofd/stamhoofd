<template>
    <div class="modern st-view table-view background" data-testid="table">
        <STNavigationBar :add-shadow="wrapColumns" :title="title" :disable-pop="true" :class="{'has-sticky-sibling': !wrapColumns}">
            <template #left>
            ...
            <template #right>
                <template v-if="!(isIOS && showSelection)">
                    <button v-for="(action, index) of filteredActions" :key="index" v-tooltip="action.tooltip" type="button" :class="'button icon '+action.icon" :disabled="action.needsSelection && ((showSelection && isMobile) || !action.allowAutoSelectAll) && !hasSelection" @click="handleAction(action, $event)" />
                </template>
                    ...
                    <div class="table-head" data-testid="table-head" @contextmenu.prevent="onTableHeadRightClick($event)">
                        ...
                        <div class="columns">
                            <div v-for="(column, index) of columns" :key="column.id" :class="{isDragging: isDraggingColumn === column && isColumnDragActive && dragType === 'order'}" :data-align="column.align">
                                ...
                                <button v-else-if="canCollapse" v-tooltip="$t(`%je`)" type="button" class="button light-gray icon collapse-left" @click="collapse" />
                            </div>
                        </div>
                    </div>
...
async function showActions(isOnTop: boolean, event: MouseEvent) {
    ...
    // displayedComponent.forceCanHaveFocus = true;
