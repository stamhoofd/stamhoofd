...
import type { FetchAllOptions, FetchLimitSettings, ObjectFetcher } from '#tables/classes/ObjectFetcher.ts';
...
export class InMemoryTableAction<T extends { id: string }> extends TableAction<T> {
    ...
    async handle(data: TableActionSelection<T>) {
        const toast: Toast = new Toast($t(`%jf`), 'spinner').setHide(null);
