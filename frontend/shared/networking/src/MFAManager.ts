...
export class MFAManager {
    ...
    static async registerPasskey(session: SessionContext, name: string | null = null, setupToken?: string): Promise<MFAEnrollmentResult | null> {
        ...
        let browserResponse: Awaited<ReturnType<typeof startRegistration>>;
        try {
            browserResponse = await startRegistration({ optionsJSON: options.options as PublicKeyCredentialCreationOptionsJSON });
        } catch (error) {
            if (
