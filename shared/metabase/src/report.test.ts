...
describe('report', () => {
    ...
    describe('definition', () => {
         ...
         * A column a koepel delivers for itself stands behind them, where the sjabloon has no cell
         * to read it in.
         */
        ...
        /**
         * Keeo delivers its stam among the leden and the ondersteunende leden who paid a lidgeld
         * among the leiding, for the same reason: the metadatafiche has no third word for either,
         * and the aansluiting the delivery reads is what says the lidgeld was paid. Both are
         * volwassenen in the categories and have to stay that way, so the aanlevering names the two
         * leeftijdsgroepen itself.
         *
        ...
        it('delivers the stam of keeo as leden and its ondersteunende leden as leiding, each counted apart', () => {
            const namesTheGroups = (tabs: ReportTab[]) => tabs.flatMap(tab => tab.cards)
                .filter(card => expressionOf(card.sql).includes("all_registrations.age_group_id = '6fc0775e-2851-4fe1-90cd-af9c74243ccd'"))
                .map(card => card.key);
            const card = cardOf(dashboards, 'jeugdbewegingen', 'deelnemers-lokale-groep');
            ...
            expect(namesTheGroups(ravotDashboards)).toEqual([]);
        });
         ...
         * neither dashboard counts such a person. Both ask, and each asks its own list -- the report
         * the koepel reads and the dataset it files with the department answer to different people,
         * so neither starts counting differently because the other did.
         */
        it('gives the aanlevering and the report a list of aansluitingen each', () => {
            for (const dashboard of dashboards.filter(dashboard => !dashboard.hidden)) {
                for (const card of dashboard.cards.filter(card => card.snippets.length > 0)) {
                    ...
                    if (read.length === 0) {
                        expect(`${dashboard.key}/${card.key}`).toMatch(/organisatie-/);
                        continue;
                    }
         ...
         * override directory, a `@` qualifier or an `except:` naming an environment nobody loads is
         * read by nothing and changes nothing, which a misspelling looks exactly like. Extend this
         * when a third platform starts counting something its own way.
         */
        it('varies for the environments it names and no others', async () => {
            ...
            for (const file of (await fs.readdir(directory)).filter(entry => entry.endsWith('.sql'))) {
                ...
                for (const match of contents.matchAll(/^--[ \t]*except:(.*)$/gm)) {
            ...
            expect([...qualifiers].sort()).toEqual(['keeo', 'ravot']);
        });
        ...
        it('stands the ULDK totals under the table, in the same columns', () => {
            const placed = layoutCards(ravotDashboards.find(dashboard => dashboard.key === 'varia')!.cards);
        ...
        it('adds up every column the ULDK table holds', async () => {
            ...
            const summed = [...cardOf(ravotDashboards, 'varia', 'uldk-totaal').sql.matchAll(/SUM\(`([^`]+)`\)/g)].map(match => match[1]);
        ...
        /**
         * Neither filter is offered any more. Which lidgelden say someone is a lid, and which kinds of
         * registration count as being one, are what the koepel reports under rather than what a reader
         * picks: a reader who narrowed either would read a figure nobody else on the platform reads,
         * and two pages open side by side could disagree about who a lid is.
         */
        it('offers no filter for the aansluiting or the kind of registration', () => {
            for (const dashboard of dashboards) {
                for (const gone of ['aansluiting', 'ingeschreven_voor']) {
                    expect(`${dashboard.key}: ${dashboard.filters.includes(gone)}`).toEqual(`${dashboard.key}: false`);
                }
            }

            expect(dashboards.flatMap(dashboard => dashboard.cards).flatMap(card => card.parameters))
                .toEqual(expect.not.arrayContaining(['aansluiting', 'ingeschreven_voor']));
        });
