-- description: Eén rij per lid per eenheid per werkjaar, over alle werkjaren tegelijk: de jaarfilter bereikt dit fragment niet, wat de trendkaarten lezen.
WITH non_platform_registrations AS (
    -- @include all-non-platform-registrations-all-years
)
-- Identical to `deduplicated-non-platform-registrations.sql` apart from the grain it reads; a test keeps the two in step.
