...
OR (
    ...
    AND NOT EXISTS (
        SELECT 1
        FROM registrations AS leeftijdsgroepinschrijving
        JOIN `groups` AS leeftijdsgroep
            ON leeftijdsgroep.id = leeftijdsgroepinschrijving.groupId
           AND leeftijdsgroep.deletedAt IS NULL
           AND leeftijdsgroep.type = 'Membership'
        WHERE leeftijdsgroepinschrijving.memberId = registrations.memberId
          AND leeftijdsgroepinschrijving.periodId = registrations.periodId
          AND leeftijdsgroepinschrijving.registeredAt IS NOT NULL
    )
)
