...
| Tests       | `pnpm stam test coverage`              | Run package coverage with isolated MySQL.                                  |
...
Development sessions use explicit Turbo filters for the API, renderer, statistics syncer, web app, and webshop. These tasks are persistent and uncached. Turbo runs in loose environment mode so values generated by the CLI, including Stripe values, reach package processes. The docs target starts Nuxt directly and does not wait for shared packages because it does not consume their output.
