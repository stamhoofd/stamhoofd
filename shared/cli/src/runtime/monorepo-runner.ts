...
export async function testE2e(context: CliContext, options: { ci: boolean; ui: boolean }): Promise<void> {
    await buildShared(context);
    await run('yarn', ['--cwd', 'backend/app/api', '-s', 'build:playwright:pre'], { cwd: context.rootDir, verbose: context.verbose });
    await run('yarn', ['--cwd', 'tests/playwright', '-s', 'test', ...(options.ui ? ['--ui'] : [])], { cwd: context.rootDir, env: { NX_DAEMON: 'false', CI: options.ci ? 'true' : undefined }, verbose: context.verbose });
}
