...
import type { ReportDashboard } from './metabase-report.js';

/**
 * Where the report definition lives: with the migration that creates the tables its queries read.
 */
export const statisticsPackagePath = path.join('backend', 'shared', 'statistics');

/**
 * Reads the report by running the statistics package, which owns the parser as well as the queries.
 *
 ...
 */
export async function loadReportDefinition(context: CliContext): Promise<ReportDashboard[]> {
    const script = path.join(context.rootDir, statisticsPackagePath, 'dist', 'print-report.js');

    if (!await exists(script)) {
        throw new Error(`The statistics package is not built, so the report definition cannot be read. Run: yarn build:shared`);
    }

    const result = await run('node', [script], { capture: true, quiet: true, verbose: context.verbose });

    try {
        return JSON.parse(result.stdout) as ReportDashboard[];
    }
    catch (error) {
