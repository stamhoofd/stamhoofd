...
import { MemberWithRegistrationsBlob, SGVSyncStatus, getSGVSyncStatus, isSGVManagedMember } from './MemberWithRegistrationsBlob.js';
...
describe('MemberWithRegistrationsBlob SGV sync helpers', () => {
    ...
    test('returns never without last external sync', () => {
        expect(getSGVSyncStatus(createMember({ lastExternalSync: null }), { now })).toBe(SGVSyncStatus.Never);
    });
    ...
    test('returns outdated when a current registration is newer than the last sync', () => {
        ...
        expect(getSGVSyncStatus(member, { now })).toBe(SGVSyncStatus.Outdated);
    });
    ...
    test('returns ok when the member update time is close to the last sync', () => {
        ...
        expect(getSGVSyncStatus(member, { now })).toBe(SGVSyncStatus.Ok);
    });
    ...
    test('returns changed when member details changed after the last sync', () => {
        ...
        expect(getSGVSyncStatus(member, { now })).toBe(SGVSyncStatus.Changed);
    });

    test('detects members managed by SGV sync', () => {
        expect(isSGVManagedMember(createMember())).toBe(true);
        expect(isSGVManagedMember(createMember({ group: createGroup(GroupType.EventRegistration) }))).toBe(false);
        expect(isSGVManagedMember(createMember({ registeredAt: null }))).toBe(false);
    });
    ...
    test('ignores registrations from other periods when a period id is provided', () => {
        ...
        expect(getSGVSyncStatus(member, { now, periodId: 'period-id' })).toBe(SGVSyncStatus.Ok);
        expect(getSGVSyncStatus(member, { now, periodId: 'other-period' })).toBe(SGVSyncStatus.Outdated);
    });
    ...
    test('marks syncs older than nine months as outdated', () => {
        ...
        }), { now })).toBe(SGVSyncStatus.Outdated);
    });
    ...
    test('can detect managed members with externally supplied groups', () => {
        ...
        expect(isSGVManagedMember(member)).toBe(false);
        expect(isSGVManagedMember(member, [createGroup(GroupType.Membership, 'period-id', 'group-id')])).toBe(true);
    });
});
