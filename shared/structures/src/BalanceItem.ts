...
const descriptionAsTitleTypes = new Map<BalanceItemType, boolean>();

/**
 * Whether this kind of item is named after its description, which is then the only thing that tells two
 * of its articles apart (e.g. 'other' items and administration costs).
 *
 ...
 */
function usesDescriptionAsTitle(type: BalanceItemType): boolean {
    let result = descriptionAsTitleTypes.get(type);

    if (result === undefined) {
        const probe = ' article-description';
        result = BalanceItem.create({ type, description: probe }).itemTitle === probe;
        descriptionAsTitleTypes.set(type, result);
    }
...
function getArticleDescriptionCode(item: BalanceItem): string {
    return usesDescriptionAsTitle(item.type) ? '-description-' + item.description : '';
}
...
export class BalanceItem extends AutoEncoder {
    ...
    get articleFilter(): StamhoofdFilter {
        ...
        if (usesDescriptionAsTitle(this.type)) {
            filters.push({ description: this.description });
        }
