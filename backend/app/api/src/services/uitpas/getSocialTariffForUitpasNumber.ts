...
export async function getSocialTariffForUitpasNumber(access_token: string, uitpasNumber: string, basePrice: number, uitpasEventId: string) {
    const baseUrl = 'https://api-test.uitpas.be/tariffs';
    const params = new URLSearchParams();
    params.append('regularPrice', (basePrice / 100).toFixed(2));
    params.append('eventId', uitpasEventId);
    ...
    if (!response.ok) {
        console.error(`UiTPAS API returned an error for UiTPAS number ${uitpasNumber} with event id ${uitpasEventId}:`, response.statusText);
        const json: unknown = await response.json().catch(() => { /* ignore */ });
        let endUserMessage = '';

        if (json) {
            console.error(`UiTPAS API returned an error for UiTPAS number ${uitpasNumber} with event id ${uitpasEventId}:`, json);
        }
        else {
            console.error(`UiTPAS API returned an error for UiTPAS number ${uitpasNumber} with event id ${uitpasEventId}:`, response.statusText);
        }
    ...
    if (json.available.length === 0) {
        if (json.endUserMessage) {
            throw new SimpleError({
                ...
                message: `No social tariff available for event ${uitpasEventId} with message`,
                human: json.endUserMessage.nl,
            });
        }
        throw new SimpleError({
            code: 'no_social_tariff_available',
            message: `No social tariff available for event ${uitpasEventId}`,
    ...
    if (json.endUserMessage) {
        console.warn('UiTPAS API returned an end user message for event', uitpasEventId, ':', json.endUserMessage.nl);
    ...
    if (json.available[0].remaining < 1) {
        throw new SimpleError({
            ...
            message: `No remaining social tariff for event ${uitpasEventId} and UiTPAS number ${uitpasNumber}`,
...
export async function getSocialTariffForUitpasNumbers(access_token: string, uitpasNumbers: string[], basePrice: number, uitpasEventId: string) {
    ...
    for (let i = 0; i < uitpasNumbers.length; i++) {
        ...
        try {
            reducedPrices[i] = await getSocialTariffForUitpasNumber(access_token, uitpasNumber, basePrice, uitpasEventId); // Throws if invalid
        }
        catch (e) {
