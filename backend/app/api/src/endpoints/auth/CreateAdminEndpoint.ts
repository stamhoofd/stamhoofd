...
export class CreateAdminEndpoint extends Endpoint<Params, Query, Body, ResponseBody> {
    ...
    async handle(request: DecodedRequest<Params, Query, Body>) {
        ...
        const name = organization?.name ?? STAMHOOFD.platformName
        const what = organization ? `de vereniging ${name} op ${STAMHOOFD.platformName}` : `${STAMHOOFD.platformName}`
