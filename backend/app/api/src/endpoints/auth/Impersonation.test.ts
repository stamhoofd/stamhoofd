...
import { AuditLog, GroupFactory, ImpersonationToken, MemberFactory, OrganizationFactory, RegistrationFactory, Token, User, UserFactory } from '@stamhoofd/models';
import { AuditLogType, BooleanStatus, MemberDetails, MemberWithRegistrationsBlob, NewUser, PermissionLevel, Permissions, PermissionsResourceType, ResourcePermissions, Token as TokenStruct, UserPermissions } from '@stamhoofd/structures';
...
describe('Impersonation', () => {
    ...
    describe('starting an impersonation', () => {
        test('a full admin gets a link and can trade it for a session', async () => {
            ...
            expect(saved!.impersonatedUserId).toBe(user.id);
        });
        ...
        test('in platform mode an organization admin can impersonate an administrator of their own organization', async () => {
            ...
            expect((await Token.getByAccessToken(token.accessToken))!.impersonatedUserId).toBe(colleague.id);
        });
    ...
    describe('the resulting session', () => {
        ...
        test('is dropped when the account changes its password', async () => {
            ...
            await Token.clearFor(user.id, 'some-other-session');
    ...
    describe('member data of the account being looked at', () => {
        /**
         * An administrator whose only permission is full access to a single group, and a
         * parent with a child registered in that group. The administrator may impersonate
         * the parent - the group gives them full access to that member - but the group
         * gives them no access to national register numbers or financial data.
         */
        async function setupGroupAdmin() {
            ...
            return { organization, group, admin, user, member };
        }

        test('an administrator does not read data through the eyes of a parent that they could not read themselves', async () => {
            const { organization, admin, user, member } = await setupGroupAdmin();

            const session = await impersonate(organization, admin, user);
            const response = await testServer.test(new GetUserMembersEndpoint(), membersRequest(organization, session));

            const blob = response.body.members.find(m => m.id === member.id);
            expect(blob).toBeDefined();

            // The parent may see all of this, the administrator may not, so the session
            // does not get it either.
            expect(blob!.details.nationalRegisterNumber).toBeNull();
            expect(blob!.details.requiresFinancialSupport).toBeNull();
        });

        test('an administrator that may read the data does see it', async () => {
            const { organization, admin, user, member } = await setupGroupAdmin();

            // Same setup, but now the administrator runs the whole organization.
            admin.permissions = UserPermissions.create({});
            admin.permissions.organizationPermissions.set(organization.id, Permissions.create({ level: PermissionLevel.Full }));
            await admin.save();
            ...
            const blob = response.body.members.find(m => m.id === member.id);
            expect(blob!.details.nationalRegisterNumber).toBe(member.details.nationalRegisterNumber);
            expect(blob!.details.requiresFinancialSupport?.value).toBe(true);
        });
