...
async function throwIfNoFinancialReadAccess() {
    ...
    if (!organization) {
        if (!Context.auth.hasPlatformFullAccess()) {
            throw new SimpleError({
                ...
                message: 'No permissions for financial support sorter.',
                human: $t(`Je hebt geen toegangsrechten om deze sortering te gebruiken.`),
                statusCode: 400,
            });
        }
        return;
    ...
    if (!permissions || !permissions.hasAccessRight(AccessRight.MemberReadFinancialData)) {
        throw new SimpleError({
            code: 'permission_denied',
            message: 'No permissions for financial support sorter (organization scope).',
            human: $t(`Je hebt geen toegangsrechten om deze sortering te gebruiken.`),
            statusCode: 400,
        });
    }
