...
export class ContextInstance {
    ...
    async authenticate({ allowWithoutAccount = false, allowUnscoped = false, allowMFASetupToken = false }: { allowMFASetupToken?: boolean; allowWithoutAccount?: boolean; allowUnscoped?: boolean } = {}): Promise<{ user: User; token: Token } | { user: User; setupToken: MFAToken }> {
        ...
        const impersonatedUser = token.impersonatedUserId ? await this.resolveImpersonation(token.impersonatedUserId, user) : null;
    ...
    async insecurelyAuthenticateAs(user: User, impersonatedUser?: User | null) {
        ...
        this.#auth = impersonatedUser
            ? new ImpersonatedPermissionChecker(impersonatedUser, user, platform, this.organization)
