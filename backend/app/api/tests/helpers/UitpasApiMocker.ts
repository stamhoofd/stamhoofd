...
export class UitpasMocker {
    ...
    start() {
        if (!STAMHOOFD.UITPAS_API_CLIENT_ID || !STAMHOOFD.UITPAS_API_CLIENT_SECRET?.startsWith('sk_test_')) {
            throw new Error('Invalid UITPAS_API_CLIENT_SECRET. Even in test mode it should start with sk_test_');
        }

        // mock access-token
        nock('https://account-test.uitid.be')
            .post(
                '/realms/uitid/protocol/openid-connect/token',
            )
            .reply(200, {
        ...
        nock('https://api-test.uitpas.be')
            ...
            .get(/\/passes\/(\d+)/).reply((uri: string) => {
                ...
                if (!result) {
                    return [400, { type: 'https://api.publiq.be/probs/uitpas/invalid-uitpas-number', title: 'Invalid uitpas number', status: 400, detail: '0900000031617', endUserMessage: { nl: 'Het UiTPAS-nummer dat je invulde konden we niet terugvinden. Kijk je het nummer even na?' } }];
                }
