...
describe('Wildcards', () => {
    ...
    it('Correctly finds the right filter when using object syntax', async () => {
        await test({
            ...
            filters,
            query: {
                query: 'JSON_VALUE(JSON_EXTRACT(JSON_EXTRACT(`default`.`settings`,"$.records"),?),"$.name" RETURNING CHAR CHARACTER SET utf8mb4 NULL ON ERROR) COLLATE utf8mb4_0900_ai_ci <=> ?',
                params: [
                    '$.a',
                    'rex',
                ],
    ...
    it('Can support this', async () => {
        await test({
            ...
            filters,
            query: {
                query: 'JSON_VALUE(JSON_EXTRACT(`default`.`settings`,"$.records"),? NULL ON ERROR) IS NOT NULL',
                params: [
                    '$.a',
                ],
            },
