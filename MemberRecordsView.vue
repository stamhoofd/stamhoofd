...
export default class MemberRecordsView extends Mixins(NavigationMixin) {
    ...
    async goNext() {
        ...
        if (this.allowData) {
            ...
            this.memberDetails.records = this.memberDetails.records.filter(r => r.type == RecordType.NoData || r.type == RecordType.NoPictures||r.type == RecordType.NoGroupPictures || r.type == RecordType.NoPermissionForMedicines )
    ...
    get allowGroupPictures() { return !this.getBooleanType(RecordType.NoGroupPictures) }
    set allowGroupPictures(enabled: boolean) { this.setBooleanType(RecordType.NoGroupPictures, !enabled) }
