...
const blackLists = new Map<TextType, Set<string>>([
    ...
    [TextType.FunctionArgument, new Set(['$t', 'process.on', 'console.error', 'console.log', 'console.info', 'console.warn', 'import', 'resolve', 'where', 'whereNot', 'Migration.runAll', 'AuditLogReplacement.key', 'loadAllEndpoints', '.set', '.includes', ' if', 'registerCron', '.column', 'require', 'setZone', 'startsWith', 'parseParameters', 'isRunning','SQLAlias', 'schedule', '.or', '.and', '.from', '.wildcard', '.table', 'fromRows', 'andWhere', 'QueueHandler.debounce', '.checkDuplicate', '.orderBy', 'Database.select', '.addClass', 'Database.statement', 'createSQLColumnFilterCompiler', 'addClassStyle', 'addClassAfterStyle'])],
