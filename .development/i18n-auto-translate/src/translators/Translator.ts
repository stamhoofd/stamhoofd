...
export abstract class Translator implements ITranslator {
    ...
    constructor(protected readonly manager: TranslationManager) {
    }

    protected abstract translateBatch(batch: string[], args: { originalLocal: string; targetLocal: string; consistentWords: Record<string, string> | null; namespace: string, batchNumber: number, totalBatches: number }): Promise<string[]>;
