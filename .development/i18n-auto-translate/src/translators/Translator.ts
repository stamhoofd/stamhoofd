...
export abstract class Translator implements ITranslator {
    ...
    constructor(protected readonly manager: TranslationManager, protected readonly cliArgs: CliArguments = cliArguments) {
    }

    protected abstract generateResponse(prompt: string): Promise<string>;

    private fakeGenerateResponse(textArray: string[], { originalLocal, targetLocal, namespace }: { originalLocal: string; targetLocal: string; namespace: string; }): Promise<string> {
        const infoText = `Fake translated from ${originalLocal} to ${targetLocal} for namespace ${namespace}: `;
        return Promise.resolve(JSON.stringify( textArray.map(text => infoText + text)));
    }

    protected abstract createPrompt(textArray: string[], {originalLocal, targetLocal, consistentWords, namespace}: { originalLocal: string; targetLocal: string; consistentWords: Record<string, string> | null, namespace: string }): string;

    protected async getTextFromApi(textArray: string[], args: { originalLocal: string; targetLocal: string; consistentWords: Record<string, string> | null, namespace: string,batchNumber: number, totalBatches: number }): Promise<string> {
        const prompt = this.createPrompt(textArray, args)
        
        const logResult = promptLogger.prompt(prompt, args);

        const text = this.cliArgs.fake ? await this.fakeGenerateResponse(textArray, args) : await this.generateResponse(prompt);
