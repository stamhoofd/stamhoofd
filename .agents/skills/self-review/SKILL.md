---
name: self-review
description: Spin up a dedicated review sub-agent to critique Codex's own pending changes or recent commits, following the adjustable methodology in this skill. Use when the user asks to review your changes, review the diff, review the last commit(s), do a self-review, or types /self-review. Optional argument = the scope to review (e.g. a commit ref, a range, "last 2 commits", "staged").
---
...
Do **not** review the code yourself in the main task — the whole point is a fresh pair of eyes. Launch **one** sub-agent with Codex's `spawn_agent` collaboration tool (task name
like `review_current_diff`). Its prompt must contain:

- The exact scope you resolved (the git command(s) that reproduce the diff, or the branch/range).
- This instruction, verbatim:
  > Read `.agents/skills/self-review/methodology.md` and follow it exactly. Review **only**
